...
class MetadataDbLinkerTool(object):
    ...
    field_def = {
        ...
        "port": {"type": "int", "required": False, "editable": True, "is_shown": False},
        "schema": {
            "type": "varchar",
            "required": False,
            "editable": True,
    ...
    def _create_db_creation_statement(self):
        ...
        b = [
            self._quote(f) + " " + self.field_def.get(f).get("type")
            for f in list(self.field_def)
        ]
    ...
    def insert(self, data={}):
        ...
        flds,vls,binds = self.data_to_flds_vls(data)
    ...
    def update(self, data={}):
        ...
        query = QtSql.QSqlQuery(db)
        # TODO: Make this work with binds (which for some reason does not work with tupled update statements?)
        query.prepare('UPDATE "{schema}"."{table}" SET ({fields}) = ({values}) WHERE guid={guid}'.format(
            schema=self.get_schema(),
            table=self.get_table(),
            fields=",".join(flds),
            values=",".join(binds),
            guid=self._single_quote(data.get("guid"))
        ))
    ...
    def select(self, d, order_by={}):
        ...
        for k in list(d):
            if self.field_def.get(k).get("type") in ["varchar"]:
                c.append("%s=%s" % (self._quote(k), self._single_quote((d.get(k)))))

        # Add in valid additional fields when selecting
        flds = list(self.field_def)
        
        try:
            if self.get_additional_fields():  
                flds += list(self.get_additional_fields().keys())
        except Exception:
    ...
    def validate_structure(self):
        ...
        s = """
            ...
            AND
              table_schema='{schema}';
            """.format(
            table=self.get_table(), schema=self.get_schema()
        )
    ...
    def validate_gui_table(self):
        ...
        s = """
            ...
            FROM
              {schema}.{table}
            """.format(
            table=self.get_gui_table(), schema=self.get_schema()
        )
        ...
        if query.exec_(s):
            while query.next():
                ...
                if not f or f is "":
                    raise Exception(
                        'Column "metadata_col_name" was empty, check gui_table configuration'
                    )
        else:
            raise Exception(
                "Could not access gui_table \n" + str(query.lastError().text())
            )
    ...
    def get_additional_fields(self):
        ...
        s2 = """
        SELECT metadata_col_name, required, editable, type, displayname, is_shown, id
        FROM {schema}.{guitable}
        WHERE is_shown
        """.format(
            guitable=self.get_gui_table(), schema=self.get_schema()
        )
    ...
    def data_to_flds_vls(self,data):
        """Turns data into list of fields and corresponding values

        Handles types and extra fields. Converts datetime into proper DB structure
        
        Arguments:
            data {dict} -- data coming from dialog_metadata on either an update or insert statement
        
        Returns:
            flds {list} -- fields (name,description, ..)
            vls {list} -- values ("Cykelsier", "Metadata for cykelstier...", ...)
            binds {list} -- binds for prepared statement
        """
        flds = []
        vls = []
        binds = []
        ...
        for k in list(data):
            ...
            try:
                ...
                if not val:
                    val = None
            except AttributeError:  # TODO: handle additional_fields
                d_type = data.get(k)["type"]
                val = data.get(k)["value"]
                if not val:
                    val = None

            #binds.append(":{}".format(k))
            binds.append("?")
            flds.append(k)
            if d_type in ["varchar", "text"]:
                vls.append(val)
            elif d_type in ["uuid"]:
                if len(val) == 0 or val == "NULL":
                    vls.append(val)  # Dont single quote
                else:
                    vls.append(val)
            else:
                vls.append(val)

        return flds,vls,binds
